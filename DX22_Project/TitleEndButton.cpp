...
void CTitleEndButton::Init()
{
	...
	m_pSEList["PlayerVoice"] = AddComponent<CAudio>();
	m_pSEList["PlayerVoice"]->Load(PATH_SE("TitlePlayer.wav"));
}
...
{
	if (!m_pSEList["PlayerVoice"]->IsPlay()) m_pSEList["PlayerVoice"]->Play(true);
